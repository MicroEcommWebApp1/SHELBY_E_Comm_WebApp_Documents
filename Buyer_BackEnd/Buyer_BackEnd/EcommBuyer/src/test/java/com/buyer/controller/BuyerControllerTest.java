...
class BuyerControllerTest {
	...
	@BeforeEach
	void setup() {
		buyerDto = BuyerDto.builder().id(1).name("Swetha").email("swetha333555@gmail.com").password("swetha08!")
				.phonenumber("9876543211").build();
	}

	@Test 
	 void buyerRegistrationTest() throws Exception {
	...
	@BeforeEach
	public void setup1() {
		login = LoginDto.builder().email("swetha333555@gmail.com").password("swetha08!").build();
	}

	@Test
	 void buyerLogin_Test() throws Exception {
	 ...
	 * when(buyerService.forgotPassword(testEmail, testPassword)).thenReturn(new
	 * ResponseEntity<>(HttpStatus.OK));
	 * mockMvc.perform(put("/buyer/forgotpassword/swetha333555@gmail.com/swetha08!")
	 * .contentType("application/json") .content(new
	 * ObjectMapper().writeValueAsString(testPassword)))
	 * .andExpect(status().isOk()); }
	 */
